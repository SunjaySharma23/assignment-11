...
def plot_category_performance_by_location():
    ...
    grouped_data = recent_data.groupby(['Location', 'Category'])['Sales'].sum().unstack()
...
def plot_ad_efficiency_over_time():
    ...
    """
    # Your code here
    ad_efficiency = sales_df.groupby('QuarterLabel')['SalesPerDollarSpent'].mean()
   
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(ad_efficiency.index, ad_efficiency.values, marker='o')
    ax.set_title("Advertising Efficiency Over Time")
    ax.set_xlabel("Quarter")
    ax.set_ylabel("Average Sales Per Dollar Spent")
    ax.grid(True)
    
    highest_quarter = ad_efficiency.idxmax()
    highest_value = ad_efficiency.max()
    ax.annotate("Highest Efficiency", (highest_quarter, highest_value))
...
def plot_customer_age_distribution():
    ...
    for i, location in enumerate(locations, start=1):
        ...
        axes[i].axvline(location_ages.mean(), linestyle='--', label='Mean')
        axes[i].axvline(location_ages.median(), linestyle= ':', label='Median')
        axes[i].legend()
...
def plot_purchase_by_age_group():
    ...
    grouped_purchases = [
        customer_df[customer_df['AgeGroup'] == '18-30']['PurchaseAmount'],
        customer_df[customer_df['AgeGroup'] == '31-45']['PurchaseAmount'],
        customer_df[customer_df['AgeGroup'] == '46-60']['PurchaseAmount'],
        customer_df[customer_df['AgeGroup'] == '61+']['PurchaseAmount']
    ]
    
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.boxplot(grouped_purchases, tick_labels=['18-30', '31-45', '46-60', '61+'])
...
def plot_sales_by_price_tier():
    ...
    largest_index = tier_sales.values.argmax()
    explode_values[largest_index] = 0.1
    
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.pie(tier_sales.values, labels=tier_sales.index, autopct='%1.1f%%', explode=explode_values, startangle=90)
...
def plot_category_market_share():
    ...
    explode_values = [0] * len(category_sales)
    largest_index = category_sales.values.argmax()
    explode_values[largest_index] = 0.1
    
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.pie(category_sales.values, labels=category_sales.index, autopct='%1.1f%%', explode=explode_values, startangle=90)
...
def main():
    ...
    print("1. Overall sales generally trend upward over time, with stronger performance in Q4.")
    print("2. Miami has the highest sales among store locations.")
    print("3. Electronics holds the largest market share among product categories.")
    print("4. There is a positive relationship between advertising spend and sales.")
    print("5. Customer age distributions vary by location, with Miami skewing younger than Tampa.")
    print("6. Mid-range and premium price tiers make up a large share of purchase value.")
    # Display all figures
    plt.show()
